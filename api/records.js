...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'POST' && action === 'bulk_fix_imports') {
    ...
    let fixed = 0;
    for (const id of ids) {
      await sql`UPDATE injection_records
        SET category = item,
            item = '',
            notes = ''
        WHERE id = ${id}`;
...
NaN
      ...
      if (action === 'save_part') {
        ...
        if (p.id) rows = await sql`UPDATE parts_inventory SET part_number=${p.part_number},description=${p.description},manufacturer=${p.manufacturer||''},category=${p.category||''},quantity=${p.quantity||0},min_quantity=${p.min_quantity||1},unit_cost=${p.unit_cost||0},location=${p.location||''},notes=${p.notes||''},updated_at=NOW() WHERE id=${p.id} RETURNING *`;
      ...
      if (action === 'update_part' || action === 'update_part_qty') {
        const {id,quantity,field,value} = body;
      ...
      if (action === 'save_cross_ref') {
        const cr = body;
        let rows;
        if (cr.id) rows = await sql`UPDATE parts_cross_ref SET part_number_a=${cr.part_number_a||''},part_number_b=${cr.part_number_b||''},manufacturer_a=${cr.manufacturer_a||''},manufacturer_b=${cr.manufacturer_b||''},description=${cr.description||''},price_a=${cr.price_a||0},price_b=${cr.price_b||0},notes=${cr.notes||''},updated_at=NOW() WHERE id=${cr.id} RETURNING *`;
        else rows = await sql`INSERT INTO parts_cross_ref (part_number_a,part_number_b,manufacturer_a,manufacturer_b,description,price_a,price_b,notes,company_id) VALUES (${cr.part_number_a||''},{cr.part_number_b||''},{cr.manufacturer_a||''},{cr.manufacturer_b||''},{cr.description||''},{cr.price_a||0},{cr.price_b||0},{cr.notes||''},{company_id}) RETURNING *`;
      ...
      if (action === 'receive_part') {
        const {order_id,task_id,parts,quantity} = body;
      ...
      if (action === 'search_manual') {
        const {query} = body;
        const rows = await sql`SELECT * FROM parts_manuals WHERE company_id=${company_id} AND (title ILIKE ${'%'+(query||'')+'%'} OR manufacturer ILIKE ${'%'+(query||'')+'%'} OR model ILIKE ${'%'+(query||'')+'%'}) ORDER BY title`;
        return res.json(rows);
      }
      if (action === 'search_parts_web') {
        const q = ((body.part_number||'') + ' ' + (body.description||'')).trim();
        return res.json({ ok: true, search_url: 'https://www.google.com/search?q='+encodeURIComponent(q+' price buy'), amazon_url: 'https://www.amazon.com/s?k='+encodeURIComponent(q), query: q });
      }
