...
function parseCSV(csv) {
  ...
  for (const line of lines) {
    ...
    while (cols.length < 14) cols.push('0');
    const name = cols[0];
    if (name.toLowerCase().includes('total') || name.toLowerCase().includes('grand')) continue;
    entries.push({
      full_name: name,
      minutes_worked: parseFloat(cols[1])||0, incoming_lbs: parseFloat(cols[2])||0,
      fillet_lbs: parseFloat(cols[3])||0, fillet_yield_pct: parseFloat(cols[4])||0,
      nugget_lbs: parseFloat(cols[5])||0, nugget_yield_pct: parseFloat(cols[6])||0,
      misccut_lbs: parseFloat(cols[7])||0, misccut_yield_pct: parseFloat(cols[8])||0,
      total_lbs: parseFloat(cols[9])||0, total_yield_pct: parseFloat(cols[10])||0,
      realtime_lbs_per_hour: parseFloat(cols[11])||0, eighthour_lbs_per_hour: parseFloat(cols[12])||0,
      hours_worked: parseFloat(cols[13])||0
    });
  }
  return entries;
}
...
module.exports = async function handler(req, res) {
  ...
  const prompt = "This is a C.A.T.2 Employee Production Report for a catfish processing facility. Extract ALL employee rows (skip the Grand Total row). DO NOT include employee numbers or trimmer codes in your output even if they appear on the form — these are intentionally excluded because the source system sometimes assigns them incorrectly. Only the employee name and production numbers.\\n\\nCRITICAL DIGIT ACCURACY: 6 vs 8 (open top vs closed loops), 0 vs 8 (oval vs two loops), 1 vs 7 (crossbar), 5 vs 6 (flat top vs curved).\\nIncoming lbs typically 200-1500, fillet 100-900. If a value seems implausible, re-examine the digit.\\n\\nReturn ONLY CSV, no header, one employee per line:\\nfull_name,minutes,incoming_lbs,fillet_lbs,fillet_pct,nugget_lbs,nugget_pct,misccut_lbs,misccut_pct,total_lbs,total_pct,realtime_lph,eighthour_lph,hours\\nNumbers only, no units. Return ONLY CSV rows.";
