...
async function extractPass(client, contentBlock, startRow, endRow) {
  const prompt = 'C.A.T.2 catfish trimmer report. Extract ONLY employee rows ' + startRow + ' to ' + endRow + '. Skip Grand Total. Return ONLY this minified JSON with no spaces or newlines: {"report_date":"YYYY-MM-DD","entries":[{"emp_number":"","full_name":"","trim_number":"","total_minutes":0,"incoming_lbs":0,"fillet_lbs":0,"fillet_yield_pct":0,"nugget_lbs":0,"nugget_yield_pct":0,"misccut_lbs":0,"misccut_yield_pct":0,"total_lbs":0,"total_yield_pct":0,"realtime_lbs_per_hour":0,"eighthour_lbs_per_hour":0,"hours_worked":0}]}';
  ...
  if (s < 0 || e < 0) throw new Error('No JSON in pass ' + startRow + '-' + endRow + ': ' + raw.substring(0, 80));
...
function flagEntries(entries) {
  ...
  return entries.map(function(entry) {
    ...
    if ((entry.realtime_lbs_per_hour || 0) < avgLph * 0.6 && (entry.realtime_lbs_per_hour || 0) > 0) flags.push({ field: 'realtime_lbs_per_hour', value: entry.realtime_lbs_per_hour, message: 'Lbs/hr below 60% of shift average - potential underperformer' });
...
module.exports = async function handler(req, res) {
  ...
  try {
    // Three passes of 10 rows each - each fits comfortably in 4096 tokens
    const [pass1, pass2, pass3] = await Promise.all([
      extractPass(client, contentBlock, 1, 10),
      extractPass(client, contentBlock, 11, 20),
      extractPass(client, contentBlock, 21, 35)
    ]);

    const reportDate = pass1.report_date || pass2.report_date || pass3.report_date;

    // Merge all entries, deduplicate by emp_number+name
    const seen = new Set();
    const allEntries = [];
    ...
    if (!allEntries.length) return res.status(422).json({ error: 'No entries extracted' });
