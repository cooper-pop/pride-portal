...
function yRenderCalc() {
  ...
  document.getElementById('widget-content').innerHTML =
    '<div class="wcard"><h3>📅 Date</h3><div class="wrow"><div class="wfield"><label>Date</label><input type="date" id="y-date" value="'+nd.getFullYear()+'-'+String(nd.getMonth()+1).padStart(2,'0')+'-'+String(nd.getDate()).padStart(2,'0')+'"/></div><div class="wfield"><label>Shift</label><select id="y-shift"><option>AM</option><option>PM</option></select></div></div></div>' +
  ...
  try {
    ...
    if (!log.length) { html += '<div class="log-empty">No entries yet.</div>'; }
      ...
      log.forEach(function(e){
        ...
        var dt = (function(s){var p=s.split('-');return new Date(p[0],p[1]-1,p[2]).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'});})(String(e.record_date));
