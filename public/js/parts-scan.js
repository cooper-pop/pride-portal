...
function partsScanInvoice() {
  ...
  var html = '<div style="background:#fff;border-radius:12px;padding:24px;max-width:560px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.4)">';
  html += '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">';
  html += '<h3 style="margin:0;font-size:1.1rem;color:#1e3a5f">&#128247; Scan Invoice</h3>';
  html += '<button onclick="document.getElementById('parts-scan-overlay').remove()" style="background:none;border:none;font-size:1.4rem;cursor:pointer;color:#64748b">&times;</button>';
  html += '</div>';
  ...
  html += '<div style="font-size:2rem">&#128193;</div><div style="font-size:.8rem;font-weight:600;color:#1e3a5f;margin-top:4px">Upload File</div>';
  html += '<div style="font-size:.75rem;color:#64748b">JPG, PNG, or photo of invoice</div></label>';
  html += '<label style="flex:1;border:2px dashed #cbd5e1;border-radius:8px;padding:20px;text-align:center;cursor:pointer" onmouseover="this.style.borderColor='#1e3a5f'" onmouseout="this.style.borderColor='#cbd5e1'">';
  ...
  html += '<div style="font-size:2rem">&#128247;</div><div style="font-size:.8rem;font-weight:600;color:#1e3a5f;margin-top:4px">Take Photo</div>';
  html += '<div style="font-size:.75rem;color:#64748b">Use camera</div></label></div>';
  html += '<div id="parts-scan-preview" style="display:none;margin-bottom:16px;text-align:center">';
  html += '<img id="parts-scan-img" style="max-width:100%;max-height:200px;border-radius:8px;border:1px solid #e2e8f0;margin-bottom:12px" />';
  html += '<div><button onclick="partsScanExtract()" style="background:#1e3a5f;color:#fff;border:none;border-radius:6px;padding:10px 24px;font-size:.9rem;cursor:pointer;font-weight:600">&#128269; Extract Invoice Data</button></div></div>';
  ...
  html += '<div style="font-size:.8rem;color:#64748b;margin-top:4px">Extracting part numbers, quantities, and pricing</div></div>';
  html += '<div id="parts-scan-results"></div></div>';
...
async function partsScanExtract() {
  ...
  try {
    ...
    document.getElementById('parts-scan-results').innerHTML =
      '<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:16px;color:#dc2626">&#10060; Error: ' +
      (err.message || 'Failed to extract invoice data') + '</div>';
  }
}
...
function partsScanShowResults(data) {
  ...
  html += '</div></div>';
  if (items.length > 0) {
    html += '<div style="font-weight:600;font-size:.85rem;color:#1e3a5f;margin-bottom:8px">Line Items Found (' + items.length + ') &mdash; Select items to add to inventory:</div>';
    html += '<div style="overflow-x:auto;margin-bottom:12px"><table style="width:100%;border-collapse:collapse;font-size:.78rem">';
    ...
    html += '<th style="padding:6px 4px;text-align:left">Part #</th><th style="padding:6px 4px;text-align:left">Description</th>';
    html += '<th style="padding:6px 4px;text-align:left">Mfr</th><th style="padding:6px 4px;text-align:right">Qty</th><th style="padding:6px 4px;text-align:right">Unit $</th>';
    ...
    items.forEach(function(item, idx) {
      ...
      html += '<td style="padding:5px 4px"><input type="text" value="' + (item.part_number||'').replace(/"/g,'&quot;') + '" data-field="part_number" data-idx="' + idx + '" style="width:90px;border:1px solid #e2e8f0;border-radius:4px;padding:2px 4px;font-size:.78rem"></td>';
      html += '<td style="padding:5px 4px"><input type="text" value="' + (item.description||'').replace(/"/g,'&quot;') + '" data-field="description" data-idx="' + idx + '" style="width:140px;border:1px solid #e2e8f0;border-radius:4px;padding:2px 4px;font-size:.78rem"></td>';
      html += '<td style="padding:5px 4px"><input type="text" value="' + (item.manufacturer||'').replace(/"/g,'&quot;') + '" data-field="manufacturer" data-idx="' + idx + '" style="width:65px;border:1px solid #e2e8f0;border-radius:4px;padding:2px 4px;font-size:.78rem"></td>';
      html += '<td style="padding:5px 4px;text-align:right"><input type="number" value="' + (item.quantity||1) + '" data-field="quantity" data-idx="' + idx + '" style="width:48px;border:1px solid #e2e8f0;border-radius:4px;padding:2px 4px;font-size:.78rem;text-align:right"></td>';
      html += '<td style="padding:5px 4px;text-align:right"><input type="number" step="0.01" value="' + (item.unit_cost||0) + '" data-field="unit_cost" data-idx="' + idx + '" style="width:62px;border:1px solid #e2e8f0;border-radius:4px;padding:2px 4px;font-size:.78rem;text-align:right"></td>';
      html += '</tr>';
    });
...
function partsScanGetSelectedItems() {
  ...
  document.querySelectorAll('.scan-item-check').forEach(function(cb) {
    ...
    row.querySelectorAll('input[data-field]').forEach(function(inp) {
      ...
      item[f] = (f === 'quantity' || f === 'unit_cost') ? parseFloat(inp.value)||0 : inp.value;
    });
    selected.push(item);
  });
  return selected;
  ...
  try {
    ...
    for (var i = 0; i < selected.length; i++) {
      ...
      var match = existing.find(function(p) {
        return p.part_number && item.part_number && p.part_number.toLowerCase() === item.part_number.toLowerCase();
      });
      if (match) {
        await apiCall('POST', '/api/parts?action=update_part', { id: match.id, field: 'quantity', value: (parseInt(match.quantity)||0) + (parseInt(item.quantity)||1) });
        updated++;
      } else {
        await apiCall('POST', '/api/parts?action=save_part', { part_number: item.part_number||'', description: item.description||'', manufacturer: item.manufacturer||'', category:'', quantity: parseInt(item.quantity)||1, min_quantity:1, unit_cost: parseFloat(item.unit_cost)||0, location:'', notes:'Added from invoice scan' });
        added++;
      }
    }
    alert('Done! ' + added + ' part(s) added' + (updated ? ', ' + updated + ' updated.' : '.'));
...
async function partsScanSaveInvoice() {
  ...
  try {
    await apiCall('POST', '/api/parts?action=save_invoice', { vendor: data.vendor||'', invoice_number: data.invoice_number||'', invoice_date: data.invoice_date||null, total_amount: data.total_amount||0, notes:'Scanned invoice', items: items });
    alert('Invoice saved!');
...
// Tab icon fix - corrects garbled emoji in Parts widget tabs
function partsFixTabIcons() {
  var f = {'Inventory':'📦 Inventory','Invoices':'🧾 Invoices','Manuals':'📖 Manuals','Cross-Ref':'🔀 Cross-Ref','Orders':'🚚 Orders'};
  document.querySelectorAll('[onclick*="partsShowTab"]').forEach(function(t) {
    var k = Object.keys(f).find(function(k) { return t.textContent.trim().includes(k); });
    if (k) { var o = t.getAttribute('onclick'); t.textContent = f[k]; if (o) t.setAttribute('onclick', o); }
  });
}
window.partsFixTabIcons = partsFixTabIcons;
if (typeof partsShowTab === 'function') {
  var _oPST = partsShowTab;
  partsShowTab = function(t) { _oPST.apply(this, arguments); setTimeout(partsFixTabIcons, 20); };
}