...
async async function trimRenderAnalytics() {
  ...
  function calcGrade(yld){
    if(yld===null||yld===undefined||isNaN(yld)) return {letter:'N/A',bg:'#94a3b8',col:'#fff'};
    if(yld>=95) return {letter:'A+',bg:'#059669',col:'#fff'};
    if(yld>=90) return {letter:'A', bg:'#10b981',col:'#fff'};
    if(yld>=85) return {letter:'B', bg:'#3b82f6',col:'#fff'};
    if(yld>=75) return {letter:'C', bg:'#f59e0b',col:'#fff'};
    if(yld>=60) return {letter:'D', bg:'#f97316',col:'#fff'};
    return {letter:'F',bg:'#ef4444',col:'#fff'};
  }

  function buildTable(rankings, teamAvg, days) {
    var periodLabel = days===7?'7':days===30?'30':days===60?'60':'365';
    var pills = [{label:'7 Day',days:7},{label:'30 Day',days:30},{label:'60 Day',days:60},{label:'YTD',days:365}];
    var html = '<div id="trim-pills" style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap">';
    pills.forEach(function(p){
      var active=p.days===days;
    ...
    html += '<h3 style="margin:0;font-size:.9rem;color:#1a3a6b;font-weight:700">&#127942; Trimmer Rankings &#8212; Last '+periodLabel+' Days &nbsp;<span style="font-weight:400;font-size:.78rem;color:#64748b">Team avg: '+teamAvg+' lbs/hr</span></h3>';
    html += '<button id="trim-print-btn" style="background:#1a3a6b;color:#fff;border:none;border-radius:6px;padding:5px 12px;font-size:.75rem;cursor:pointer">&#128438; Print</button>';
    html += '</div>';
    html += '<div style="overflow-x:auto"><table id="trim-rank-table" style="width:100%;border-collapse:collapse;font-size:.77rem">';
    html += '<thead><tr style="background:#1a3a6b;color:#fff">';
    ['#','Name','Days','Avg Lbs/Hr','8Hr Lbs/Hr','Fillet%','Nugget%','MiscCut%','Tot Yield%','Grade','Trend'].forEach(function(h,i){
      html += '<th style="padding:7px 8px;text-align:'+(i<=2?'center':'left')+';white-space:nowrap">'+h+'</th>';
    });
    html += '</tr></thead><tbody>';
    rankings.forEach(function(r,i){
      ...
      var trendCol = r.trend==='up'?'#059669':r.trend==='down'?'#ef4444':'#f59e0b';
      var rowBg = i%2===0?'#fff':'#f8fafc';
      var nm = r.full_name||r.name||'';
      var sid = nm.replace(/[^a-zA-Z0-9]/g,'_')+'_'+i;
      html += '<tr style="border-top:1px solid #e2e8f0;background:'+rowBg+'">';
      html += '<td style="padding:6px 8px;text-align:center;font-weight:700;color:#1a3a6b">'+(i+1)+'</td>';
      html += '<td style="padding:6px 8px;font-weight:600;color:#1a3a6b">'+nm+'</td>';
      html += '<td style="padding:6px 8px;text-align:center">'+(r.days_worked||r.days||'')+'</td>';
      html += '<td style="padding:6px 8px;font-weight:700;color:'+(parseFloat(r.avg_lbs_hr||0)>=teamAvg?'#059669':'#ef4444')+'">'+(r.avg_lbs_hr||'')+'</td>';
      html += '<td style="padding:6px 8px">'+(r.eight_hr_lbs_hr||r['8hr_lbs_hr']||'')+'</td>';
      html += '<td style="padding:6px 8px">'+(r.fillet_pct!=null?r.fillet_pct+'%':'')+'</td>';
      html += '<td style="padding:6px 8px">'+(r.nugget_pct!=null?r.nugget_pct+'%':'')+'</td>';
      html += '<td style="padding:6px 8px">'+(r.misc_cut_pct!=null?r.misc_cut_pct+'%':'')+'</td>';
      html += '<td style="padding:6px 8px;font-weight:600">'+(yld?yld+'%':'')+'</td>';
      html += '<td style="padding:6px 8px"><span style="display:inline-block;min-width:32px;text-align:center;padding:2px 7px;border-radius:20px;font-weight:800;font-size:.78rem;background:'+g.bg+';color:'+g.col+'">'+g.letter+'</span></td>';
      html += '<td style="padding:6px 8px"><button class="trim-trend-btn" data-sid="'+sid+'" data-nm="'+nm+'" data-yld="'+yld+'" data-avg="'+(r.avg_lbs_hr||0)+'" data-gl="'+g.letter+'" data-gbg="'+g.bg+'" data-gcol="'+g.col+'" style="border:none;background:none;cursor:pointer;font-size:1.1rem;color:'+trendCol+';font-weight:700;padding:2px 6px;border-radius:4px">'+trendIcon+'</button></td>';
      html += '</tr>';
      html += '<tr id="bd-'+sid+'" style="display:none"><td colspan="11" style="padding:0"><div class="trim-bd-box" style="padding:12px 16px;background:#eff6ff;border-left:4px solid #1a3a6b"></div></td></tr>';
    });
    ...
    if(!document.getElementById('tps')){
      ...
      s.innerHTML='@media print{body *{visibility:hidden}#trim-print-area,#trim-print-area *{visibility:visible}#trim-print-area{position:fixed;top:0;left:0;width:100%;font-size:11px}}';
      document.head.appendChild(s);
    }
    return html;
  }

  apiCall('GET', '/api/analytics?type=rankings&days='+window._trimPeriod)
    .then(function(data){
      var rankings = data.rankings||data||[];
      var teamAvg = data.team_avg_lbs_hr || (rankings.length ? Math.round(rankings.reduce(function(s,r){return s+(parseFloat(r.avg_lbs_hr)||0);},0)/rankings.length*10)/10 : 0);
      el.innerHTML = buildTable(rankings, teamAvg, window._trimPeriod);

      // Print button
      var pb = document.getElementById('trim-print-btn');
      ...
      // Trend buttons — expand breakdown + AI suggestion
      document.querySelectorAll('.trim-trend-btn').forEach(function(btn){
        btn.addEventListener('click', function(){
          ...
          var avg=parseFloat(this.dataset.avg), gl=this.dataset.gl;
          var gbg=this.dataset.gbg, gcol=this.dataset.gcol;
          var row=document.getElementById('bd-'+sid);
          var box=row?row.querySelector('.trim-bd-box'):null;
          if(!row||!box) return;
          if(row.style.display!=='none'){row.style.display='none';return;}
          row.style.display='';
          box.innerHTML='<span style="color:#94a3b8">&#x2728; Generating AI coaching...</span>';
          var prompt='You are a catfish processing plant coach. Trimmer "'+nm+'" has a '+yld+'% total yield at '+avg+' lbs/hr (grade '+gl+'). Give 2-3 specific, actionable improvement suggestions. Be concise. Number them.';
          fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:350,messages:[{role:'user',content:prompt}]})})
            ...
            .then(function(d){
              ...
              box.innerHTML=
                '<div id="trim-print-area" style="display:flex;gap:14px;flex-wrap:wrap;align-items:flex-start">'+
                '<div style="flex:0 0 auto;text-align:center">'+
                  '<div style="width:60px;height:60px;border-radius:50%;background:'+gbg+';display:flex;align-items:center;justify-content:center;font-size:1.5rem;font-weight:900;color:'+gcol+'">'+gl+'</div>'+
                  '<div style="font-size:.68rem;color:#64748b;margin-top:3px">Grade</div>'+
                '</div>'+
                '<div style="flex:1;min-width:180px">'+
                  '<strong style="color:#1a3a6b;display:block;margin-bottom:4px;font-size:.83rem">'+nm+' — AI Coaching</strong>'+
                  '<div style="font-size:.75rem;color:#374151;margin-bottom:6px">Yield: <strong>'+yld+'%</strong> &nbsp;|&nbsp; Avg: <strong>'+avg+' lbs/hr</strong></div>'+
                  '<div style="font-size:.77rem;line-height:1.6;color:#374151">'+text.replace(/\n/g,'<br>')+'</div>'+
                '</div>'+
                '<div style="flex:0 0 auto"><button onclick="window.print()" style="background:#1a3a6b;color:#fff;border:none;border-radius:6px;padding:5px 12px;font-size:.73rem;cursor:pointer">&#128438; Print</button></div>'+
                '</div>';
            })
